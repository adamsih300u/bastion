"""
Weather Request Analyzer - Roosevelt's "Intelligence Gathering" Module
...
class WeatherRequestAnalyzer:
    ...
    async def _extract_location(self, user_message: str, shared_memory: Dict[str, Any]) -> str:
        ...
        try:
            # ROOSEVELT'S LLM-POWERED LOCATION EXTRACTION - No more string matching hanky-panky!
    ...
    async def _extract_location_with_llm(self, user_message: str, shared_memory: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Use OpenRouter for LLM location extraction
            ...
            # ROOSEVELT'S CONTEXTUAL INTELLIGENCE: Get comprehensive conversation context
...
Return ONLY the location string, nothing else."""
            ...
            response = await client.chat.completions.create(
                model=settings.FAST_MODEL,  # Use configured fast model for location extraction
    ...
    async def _extract_location_fallback(self, user_message: str, shared_memory: Dict[str, Any]) -> str:
        """Fallback to basic patterns if LLM fails - Roosevelt's emergency protocols only!"""
    ...
    async def _get_fallback_location(self, shared_memory: Dict[str, Any]) -> str:
        ...
        # Import config for default location
        from config import settings
        if hasattr(settings, 'WEATHER_DEFAULT_LOCATION') and settings.WEATHER_DEFAULT_LOCATION:
            return settings.WEATHER_DEFAULT_LOCATION
        
        # ROOSEVELT'S POLITE INQUIRY: Ask user instead of guessing
    ...
    def update_shared_memory(self, shared_memory: Dict[str, Any], request: Dict[str, Any], weather_data: Dict[str, Any], agent_name: str) -> None:
        ...
        try:
            ...
            # ROOSEVELT'S SHARED MEMORY FIX: Use direct shared memory updates
    ...
    def _build_conversation_context(self, shared_memory: Dict[str, Any]) -> str:
        """ROOSEVELT'S CONTEXTUAL INTELLIGENCE: Build comprehensive conversation context for location extraction"""
        try:
            context_parts = []
