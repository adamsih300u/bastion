"""
Simple Intent Classification Service - Roosevelt's "Lean Routing Command"

**BULLY!** Simple, focused intent classification - no complex JSON cavalry charges!
Just get the intent, route to agent, check permission. That's it!
"""
...
class SimpleIntentService:
	"""
	Roosevelt's Simple Intent Classification Service
	
	**BULLY!** Get intent, route to agent, done! No complex capability analysis!
	"""
	...
	async def classify_intent(
		...
		"""
		**BULLY!** Simple intent classification - just the essentials!
		
		Returns: SimpleIntentResult with agent, permission, confidence
		"""
		try:
			...
			# **ROOSEVELT FIX**: Use user-configured classification model from Settings, not hardcoded
			...
			response = await chat_service.openai_client.chat.completions.create(
				...
				model=classification_model,  # Use Settings panel configured fast model
				temperature=0.1,
			...
			# Simple fallback
			return self._create_simple_fallback(user_message)
	...
	def _build_simple_prompt(self, user_message: str, conversation_context: Dict[str, Any]) -> str:
		...
		# **ROOSEVELT'S CONTEXT-AWARE AGENT FILTERING**: Only show available agents based on page context
		...
		# **ROOSEVELT'S DYNAMIC AGENT LIST**: Build available agents based on context
...
- **chat_agent**
  ...
  - TRIGGERS: "do you see", "show me", "what's in", "how do I", "explain"
...
"""
	...
	async def _parse_simple_response(self, response: str, user_message: str, conversation_context: Optional[Dict[str, Any]]) -> SimpleIntentResult:
		...
		try:
			...
			try:
				shared = (conversation_context or {}).get('shared_memory', {}) if 'conversation_context' in locals() else {}
			except Exception:
				shared = {}
			try:
				active_editor = shared.get('active_editor', {}) or {}
				...
				# **ROOSEVELT'S ACTION INTENT ROUTING**: Action intent determines agent, not just editor type
				if doc_type:
					...
					# QUERY intent → TRUST LLM DECISION (research_agent vs chat_agent)
						...
						# **ROOSEVELT'S "TRUST THE LLM" DOCTRINE**: 
