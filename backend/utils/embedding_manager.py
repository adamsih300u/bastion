...
class EmbeddingManager:
    ...
    def __init__(self):
        ...
        self.openrouter_client = None  # For query expansion
        self.qdrant_client = None
        self.query_expansion_cache = {}  # Cache for query expansions
        self.expansion_cache_ttl = 3600  # Cache TTL in seconds (1 hour)
        self.expansion_cache_timestamps = {}  # Track when expansions were cached
    ...
    async def initialize(self):
        ...
        # Initialize OpenRouter client for query expansion
    ...
    async def search_similar(
        ...
        use_query_expansion: bool = False,  # ROOSEVELT'S CHANGE: Default to False - agents handle expansion
        ...
        """
        Enhanced search with intelligent query expansion - searches both global and user collections
        
        **ROOSEVELT METADATA FILTERING**: Filter search results by category and tags!
        ...
        try:
            if user_id:
                ...
                results = await self._hybrid_search(query_text, user_id, limit, score_threshold, use_query_expansion, expansion_model)
            else:
                # Search only global collection
                results = await self._search_collection(settings.VECTOR_COLLECTION_NAME, query_text, limit, score_threshold, use_query_expansion, expansion_model)
    ...
    async def _hybrid_search(
        ...
        try:
            ...
            # Set user collection name for hybrid simple search
            self.user_collection_name = self._get_user_collection_name(user_id)
            
            if use_query_expansion:
                # Try hybrid simple search first across both collections
                simple_results = await self._hybrid_simple_search(query_text, limit, score_threshold)
                
                # If we get good results, don't waste time on expansion
                if len(simple_results) >= min(limit // 2, 10):
                    logger.info(f"✅ Hybrid simple search found {len(simple_results)} results - skipping expansion")
                    return simple_results
                
                logger.info(f"🔍 Hybrid simple search found {len(simple_results)} results - proceeding with enhanced search")
            
            # Search both collections in parallel with enhanced search
            ...
            return await self._search_collection(settings.VECTOR_COLLECTION_NAME, query_text, limit, score_threshold, use_query_expansion, expansion_model)
    ...
    async def _search_collection(
        ...
        try:
            ...
            if use_query_expansion:
                ...
                simple_results = await self._simple_vector_search(query_text, limit, score_threshold, collection_name)
                
                # If we get good results, don't waste time on expansion
                if len(simple_results) >= min(limit // 2, 10):
                    return simple_results
                
                # Try with expansion if simple search didn't yield enough results
                return await self._enhanced_search_with_expansion(query_text, limit, score_threshold, expansion_model, collection_name)
            else:
    ...
    async def _enhanced_search_with_expansion(
        self, 
        query_text: str, 
        limit: int, 
        score_threshold: float,
        ...
        """Search using query expansion for better recall in specified collection"""
        try:
            if collection_name is None:
                collection_name = settings.VECTOR_COLLECTION_NAME
            
            # Generate query expansions with specified model
            expanded_queries = await self._generate_query_expansions(query_text, expansion_model)
            ...
            logger.info(f"🔍 Executing {len(all_queries)} queries in parallel")
            
            # Generate embeddings for all queries in parallel
            all_query_texts = [query for query in all_queries]
            all_embeddings = await self.generate_embeddings(all_query_texts)
            
            # Create parallel search tasks
            async def search_single_query(query: str, query_vector: List[float], query_index: int) -> List[Dict[str, Any]]:
                """Search with a single query variation"""
                try:
                    logger.info(f"🔍 Parallel search {query_index+1}: '{query[:50]}...'")
                    ...
                    # Format results with query source info
                    formatted_results = []
                    for result in search_results:
                        formatted_results.append({
                            ...
                            "query_source": "original" if query_index == 0 else f"expansion_{query_index}",
                            "source_query": query
                        })
                    ...
                    logger.error(f"❌ Parallel search failed for query {query_index+1}: {e}")
                    return []
            
            # Execute all searches in parallel
            search_tasks = [
                search_single_query(query, embedding, i) 
                for i, (query, embedding) in enumerate(zip(all_queries, all_embeddings))
            ]
            ...
            # Combine and deduplicate results
            for query_results in parallel_results:
                if isinstance(query_results, Exception):
                    logger.error(f"❌ Parallel search task failed: {query_results}")
                    continue
            ...
            logger.info(f"🔍 Enhanced search found {len(final_results)} unique chunks from {len(expanded_queries)+1} queries")
            if final_results:
                logger.info(f"📈 Score range: {final_results[0]['score']:.3f} to {final_results[-1]['score']:.3f}")
    ...
    async def _simple_vector_search(
        ...
        """Simple vector search without expansion in specified collection"""
        try:
            if collection_name is None:
                collection_name = settings.VECTOR_COLLECTION_NAME
            ...
            results = []
            for result in search_results:
                # Extract metadata and enrich with document-level fields
                metadata = result.payload.get("metadata", {})
                
                # Add document title, author, and filename if available in payload
                if "document_title" in result.payload:
                    metadata["title"] = result.payload["document_title"]
                if "document_author" in result.payload:
                    metadata["author"] = result.payload["document_author"]
                if "document_filename" in result.payload:
                    metadata["filename"] = result.payload["document_filename"]
            ...
            logger.info(f"🔍 Simple search found {len(results)} chunks above threshold {score_threshold} in collection {collection_name}")
            return results
            
        except Exception as e:
            logger.error(f"❌ Simple vector search failed in collection {collection_name}: {e}")
            return []
    
    def _get_cached_expansions(self, query_text: str) -> List[str]:
        ...
        if query_text in self.query_expansion_cache:
            ...
            if current_time - cache_time < self.expansion_cache_ttl:
                logger.info(f"🔍 Using cached query expansions for: '{query_text[:50]}...'")
    ...
    def _cache_expansions(self, query_text: str, expansions: List[str]):
        ...
        logger.info(f"🔍 Cached {len(expansions)} query expansions for: '{query_text[:50]}...'")
    ...
    async def _generate_query_expansions(self, query_text: str, expansion_model: str = None) -> List[str]:
        ...
        try:
            ...
            if not self.openrouter_client:
                logger.warning("⚠️  OpenRouter client not initialized, skipping query expansion")
                return []
            
            # Use provided model or fall back to fast model (if configured)
            model_to_use = expansion_model or settings.FAST_MODEL
            if not model_to_use:
                logger.warning("⚠️ No model specified for query expansion and no fast model configured")
                return []
            
            # Enhanced prompt for better document discovery
            expansion_prompt = f"""Create 2 alternative search queries for: "{query_text}"
...
Return only a JSON array of the 2 alternative queries:
["alternative query 1", "alternative query 2"]"""

            logger.info(f"🔍 Calling OpenRouter with model: {model_to_use}")
            logger.info(f"🔍 Expansion prompt being sent:")
            logger.info(f"🔍 System: You generate alternative search queries. Return only a valid JSON array with no additional text or explanations.")
            logger.info(f"🔍 User prompt: {expansion_prompt}")
            ...
            response = await asyncio.wait_for(
                self.openrouter_client.chat.completions.create(
                    ...
                    max_tokens=300,  # Reduced from 1000 since we only need a few queries
                    temperature=0.7
                ),
                timeout=30.0  # Reduced from 300s to 30s for faster response
            )
            
            logger.info(f"🔍 OpenRouter response received")
            
            # Detailed logging of the exact response
            logger.info(f"🔍 Full response object: {repr(response)}")
            logger.info(f"🔍 Response.choices: {response.choices}")
            if response.choices:
                logger.info(f"🔍 First choice: {repr(response.choices[0])}")
                if hasattr(response.choices[0], 'message'):
                    logger.info(f"🔍 Message object: {repr(response.choices[0].message)}")
                    ...
                    if response.choices[0].message.content is not None:
                        logger.info(f"🔍 Content length: {len(response.choices[0].message.content)}")
                        logger.info(f"🔍 Content repr: {repr(response.choices[0].message.content)}")
            
            if not response.choices or not response.choices[0].message.content:
                logger.warning("🔍 OpenRouter returned empty response, skipping query expansion")
                return []
            
            expansions_text = response.choices[0].message.content.strip()
            logger.info(f"🔍 Raw expansions text: '{expansions_text}'")
            logger.info(f"🔍 Raw expansions repr: {repr(expansions_text)}")
            
            if not expansions_text:
                logger.warning("🔍 OpenRouter returned empty content, skipping query expansion")
                return []
            
            # Parse JSON array format using Pydantic validation
            try:
                import json
                ...
                logger.warning(f"🔍 Failed to validate expansions with Pydantic: {e}")
                expansions = []
            
            logger.info(f"🔍 Parsed expansions: {expansions}")
            
            # Filter out expansions that are too similar to original or empty
            filtered_expansions = []
            ...
            logger.info(f"🔍 Filtered expansions: {filtered_expansions}")
            final_expansions = filtered_expansions[:5]  # Limit to 5 expansions
    ...
    async def _get_document_context(self) -> str:
        """Get context about available documents for better query expansion"""
        try:
            # Get a sample of document metadata to understand content types
            ...
            if not sample_results[0]:  # No documents
            ...
            for point in sample_results[0]:
                ...
                doc_types.add(chunk_type)
                
                # Sample some content for theme detection
            ...
            all_content = " ".join(content_themes).lower()
            if "bob hope" in all_content:
                context_parts.append("Contains Bob Hope quotes and humor")
            if "chapter" in all_content:
                context_parts.append("Contains book/novel content")
            if "email" in all_content or "from:" in all_content:
                context_parts.append("Contains email content")
            if "shipit" in all_content:
                context_parts.append("Contains ShipIt company/shipping content")
            
            return "; ".join(context_parts) if context_parts else "General documents"
    ...
    async def close(self):
        ...
        cache_stats = self.get_expansion_cache_stats()
        logger.info(f"📊 Query expansion cache stats: {cache_stats}")
