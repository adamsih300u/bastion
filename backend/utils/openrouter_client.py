...
from utils.llm_reasoning_utils import add_reasoning_to_extra_body

logger = logging.getLogger(__name__)

# OPTION: Monkey-patch AsyncOpenAI (NOT RECOMMENDED - fragile!)
# This would modify AsyncOpenAI globally, but breaks on library updates
# Uncomment to enable monkey-patching instead of using wrapper:
"""
_original_create = AsyncOpenAI.chat.completions.create

async def _patched_create(self, *args, model=None, extra_body=None, **kwargs):
    if settings.REASONING_ENABLED:
        extra_body = add_reasoning_to_extra_body(extra_body=extra_body, model=model)
    return await _original_create(self, *args, model=model, extra_body=extra_body, **kwargs)

AsyncOpenAI.chat.completions.create = _patched_create
"""
...
class ChatCompletionsWrapper:
    ...
    async def create(self, *args, model: Optional[str] = None, extra_body: Optional[Dict[str, Any]] = None, **kwargs):
        ...
        - REASONING_ENABLED is True in settings
