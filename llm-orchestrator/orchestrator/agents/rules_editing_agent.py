...
class RulesEditingAgent(BaseAgent):
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "   - **PREFER BULLET POINTS OVER PARAGRAPHS**: Rules should be concise and scannable. Use bullet points to break down concepts into clear, digestible points rather than long paragraphs.\n"
            "   - Example: User says 'Magic requires components' → Expand to:\n"
            "     * Magic requires physical components that are consumed during casting\n"
            "     * Components must be gathered from the environment or purchased from merchants\n"
            "     * Different spell types require different component categories (organic, mineral, elemental)\n"
            "     * Components degrade over time if not stored in enchanted containers\n"
            "     * Rare components are expensive and difficult to obtain, limiting access to powerful magic\n"
            "   - Example: User says 'Time period is medieval' → Expand to:\n"
            "     * Time period: medieval (approximately 1000-1500 CE equivalent)\n"
            "     * Technology: basic metallurgy, agriculture, simple mechanical devices\n"
            "     * No gunpowder or advanced machinery exists\n"
            "     * Social structure: feudal hierarchy (nobility, clergy, commoners)\n"
            "     * Transportation: primarily by horse, cart, or foot\n"
            "     * Communication: slow, relies on messengers or word-of-mouth\n"
            "   - Use paragraphs ONLY when a concept requires narrative explanation or complex relationships that can't be broken into discrete points\n"
            "   - The goal is to create comprehensive, scannable rules that go beyond the user's initial concept\n\n"
            ...
            if current_request:
                if is_question:
                    request_with_instructions = (
                        ...
                        "- Transform brief concepts into comprehensive, well-organized bullet points that go beyond the initial idea\n"
                        "\n"
                    ...
                    request_with_instructions = (
                        ...
                        "- Transform brief concepts into comprehensive, well-organized bullet points that go beyond the initial idea\n"
                        "\n"
                        "Examples of creative expansion (using bullet points):\n"
                        "- User: 'Magic requires components' → You:\n"
                        "  * Magic requires physical components that are consumed during casting\n"
                        "  * Components must be gathered from the environment or purchased from specialized merchants\n"
                        "  * Different spell types require different component categories (organic, mineral, elemental)\n"
                        "  * Components degrade over time if not stored in enchanted containers\n"
                        "  * Rare components are expensive and difficult to obtain, limiting access to powerful magic\n"
                        "  * Component quality affects spell effectiveness - inferior components may cause spell failure or reduced potency\n"
                        "- User: 'Time period is medieval' → You:\n"
                        "  * Time period: medieval (approximately 1000-1500 CE equivalent)\n"
                        "  * Technology: basic metallurgy, agriculture, simple mechanical devices\n"
                        "  * No gunpowder or advanced machinery exists\n"
                        "  * Social structure: feudal hierarchy (nobility, clergy, commoners)\n"
                        "  * Transportation: primarily by horse, cart, or foot\n"
                        "  * Communication: slow, relies on messengers or word-of-mouth\n"
                        "  * Education: most people are illiterate; education limited to upper classes and clergy\n"
                        "\n"
